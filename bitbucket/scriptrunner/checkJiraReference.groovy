...
public boolean checkJiraReference(String jiraKey, Person user){
    try {
	    ...
	    return checkStatus(json.fields.status.name as String) && checkCommitAuthor(user, json.fields.assignee as Map)
	    } catch(Exception e){
        return false;
    }
}
...
for(RefChange refChange in refChanges){
    ...
    for (Commit commit in commits.getValues()) {
        ...
        for (String reference in jiraReferences){
            if(!checkJiraReference(reference, commit.author)){
		stringBuilder.append(commit.getMessage() + " must reference a JIRA issue in progress and assigned to the committer\n");
		accepted = false;
            } else {
                stringBuilder.append(commit.getMessage() + " is OK!\n");
	    }
        }
    }
}

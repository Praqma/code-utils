...
issueLinkManager.getOutwardLinks(issue.id)?.each {issueLink ->
    if (issueLink.issueLinkType.name == "Epic-Story Link" ) {
		def customFieldSP = ComponentAccessor.getCustomFieldManager().getCustomFieldObject("customfield_10006");
        double SP = (double)(issueLink.destinationObject.getCustomFieldValue(customFieldSP) ?: 0)
        logger.info("Issue link type: " + issueLink.issueLinkType.name + " : OK  : SP=" + SP )
        totalSP = SP + totalSP;
    } else {
